...
func TestToKebabWithIgnore(t *testing.T) {
	cases := [][]string{
		{"testCase", "test-case"},
		{"TestCase", "test-case"},
		{"Test Case", "test-case"},
	...
	for _, i := range cases {
		...
		result := ToKebabWithIgnore(in, ignore)
		if result != out {
			istr := ""
			if len(i) == 3 {
				istr = " ignoring '" + i[2] + "'"
...
func BenchmarkToKebabWithIgnore(b *testing.B) {
	for n := 0; n < b.N; n++ {
		ToKebabWithIgnore("awesome-activity.user-id", ".")
	}
}
